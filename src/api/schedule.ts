...
const generateFairGreedySchedule = async (
  data: DateRange
  ...
  try {
    ...
    if (error instanceof AxiosError) {
      throw new Error(
        error.response?.data.message || "Error generating fair greedy schedule"
      );
    }
...
const generateBasicGreedySchedule = async (
  data: DateRange
): Promise<ScheduleResult[]> => {
  try {
    ...
    if (error instanceof AxiosError) {
      throw new Error(
        error.response?.data.message || "Error generating basic greedy schedule"
      );
    }
    throw new Error("Unexpected error occurred");
  }
...
const generateRoundRobinSchedule = async (
  ...
  try {
    ...
    const response = await api.post<{
      message: string;
      result: ScheduleResult[];
    }>("/schedule/round-robin", data);
    return response.data.result;
  } catch (error) {
    if (error instanceof AxiosError) {
      throw new Error(
        error.response?.data.message || "Error generating round robin schedule"
      );
    }
    throw new Error("Unexpected error occurred");
  }
...
const generateRandomSchedule = async (
  ...
  try {
    ...
    const response = await api.post<{
      message: string;
      result: ScheduleResult[];
    }>("/schedule/random", data);
    return response.data.result;
  } catch (error) {
    if (error instanceof AxiosError) {
      throw new Error(
        error.response?.data.message || "Error generating random schedule"
      );
    }
    throw new Error("Unexpected error occurred");
  }
...
const assignSingleSchedule = async (
  data: AssignScheduleData
): Promise<Schedule> => {
  try {
    ...
    if (error instanceof AxiosError) {
      throw new Error(
        error.response?.data.message || "Error assigning schedule"
      );
    }
    throw new Error("Unexpected error occurred");
  }
...
const compareSchedules = async (data: DateRange): Promise<CompareResponse> => {
  try {
    ...
    const response = await api.get<CompareResponse>(
      `/schedule/evaluate?weekStart=${data.weekStart}`
    );
    return response.data;
  } catch (error) {
    if (error instanceof AxiosError) {
      throw new Error(
        error.response?.data.message || "Error comparing schedules"
      );
    }
    throw new Error("Unexpected error occurred");
  }
...
const getMySchedules = async (weekStart?: string): Promise<Schedule[]> => {
  try {
    ...
    if (error instanceof AxiosError) {
      throw new Error(
        error.response?.data.message || "Error fetching my schedules"
      );
    }
    throw new Error("Unexpected error occurred");
  }
